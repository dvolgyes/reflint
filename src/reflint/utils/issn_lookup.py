...
class ISSNLookupService:
    ...
    async def lookup_issn_by_journal_name(self, journal_name: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            if openalex_result and (openalex_result.get("issn_l") or openalex_result.get("issn")):
                result = self._process_openalex_result(openalex_result)
                if result:
                    logger.debug(f"Found ISSN via OpenAlex for '{journal_name}': {result}")
                    return result
    ...
    def _process_openalex_result(self, openalex_result: dict) -> dict[str, Any] | None:
...
async def lookup_journal_issn(journal_name: str, email: str | None = None) -> dict[str, Any] | None:
